module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/pillow.png -i assets/pillowmask.png -i assets/white.png -filter_complex "[1:v][0:v]scale2ref[pillow][image];[2:v][image]scale2ref[pillowmask][iimage];[3:v][iimage]scale2ref=w=iw*1.25:h=ih+(ow-iw)[white][iiimage];[iiimage][pillowmask]overlay=x=0:y=0:format=auto[imagemask];[imagemask][pillow]overlay=x=0:y=0:format=auto[pillow];[white][pillow]overlay=x=(W-w)/2:y=(H-h)/2:format=auto[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} ${filepath}/output.png`)
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/pillow.png -i assets/pillowmask.png -i assets/white.png -map 0:a? -filter_complex "[1:v][0:v]scale2ref[pillow][image];[2:v][image]scale2ref[pillowmask][iimage];[3:v][iimage]scale2ref=w=iw*1.25:h=ih+(ow-iw)[white][iiimage];[iiimage][pillowmask]overlay=x=0:y=0:format=auto[imagemask];[imagemask][pillow]overlay=x=0:y=0:format=auto[pillow];[white][pillow]overlay=x=(W-w)/2:y=(H-h)/2:format=auto[oout];[oout]scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
