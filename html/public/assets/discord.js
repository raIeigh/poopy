...
function format(value) {
    var formatters = [
        {
            ...
            format: (s) => {
                var language = ''
                var split = s.substring(s.startsWith('\n') ? 4 : 3, s.length - 3).split('\n')
        ...
        {
            wholeMatch: /\[[\s\S]+]\((http|https):\/\/[^\s`"]+\)/,
            format: (hyperlink) => {
                var hypername = hyperlink.match(/\[[^<`]+]/)[0]
    ...
    formatters.forEach(formatter => {
        ...
        function formatAllNodes(element, formatter) {
            var datas = []
            var formatDatas = []
            var matchIndex = -1
            var nmatch = -1
            ...
            element.childNodes.forEach((node) => {
                ...
                if (node.data != undefined) {
                    if (formatter.globalMatch) {
                        formatDatas.push(datas[n].replace(formatter.globalMatch, formatter.format))
                    }

                    if (formatter.twemoji) {
                        formatDatas.push(twemoji.replace(datas[n], (emoji) => `<img class="emoji" alt="${emoji}" src="https://twemoji.maxcdn.com/v/14.0.2/svg/${twemoji.convert.toCodePoint(emoji)}.svg">`))
                    }
                    ...
                    if (formatter.match || formatter.wholeMatch) {
                        ...
                        while (i < datas[n].length) {
                            ...
                            if (formatter.wholeMatch) {
                                ...
                                if (match) {
                                    formatDatas.push(datas[n].replace(match[0], formatter.format(match[0])))
                                    i += match[0].length
                                    continue
                                }
                            } else {
                                var match = current.match(new RegExp(`^${formatter.match.source}`))

                                if (match) {
                                    if (matchIndex > -1 && nmatch > -1) {
                                        var slice = datas.slice(nmatch, n).join('')
                                        var replace = (slice + datas[n]).substring(matchIndex, slice.length + i + match[0].length)

                                        formatDatas.push(formatter.format(replace))
                                        matchIndex = -1
                                        nmatch = -1
                                    } else {
                                        matchIndex = i
                ...
                } else if (!noParse.find(no => (node.tagName ?? '').toLowerCase() == no)) datas[n] = formatAllNodes(node, formatter)
...
document.addEventListener('DOMContentLoaded', () => {
    var elements = document.querySelectorAll('div.message div.contents')
