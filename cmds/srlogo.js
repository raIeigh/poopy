module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/soupremastered.png -filter_complex "[1:v]scale=${width}/3:-1[soup];[0:v][soup]overlay=x=0:y=H-h:format=auto[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} ${filepath}/output.png`)
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/soupremastered.png -map 0:a? -filter_complex "[1:v]scale=${width}/3:-1[soup];[0:v][soup]overlay=x=0:y=H-h:format=auto,scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
