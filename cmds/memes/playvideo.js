module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -stream_loop -1 -i assets/playvideo.mp4 -map 0:a? -filter_complex "${height >= 224 ? `[1:v]scale=-1:${height}[ckout];` : ''}[${height >= 224 ? `ckout` : `1:v`}]colorkey=0x66BB24:0.3:0.3[jout];[0:v][jout]overlay=shortest=1:x=W/2-w/2:y=H-h:format=auto[sout];[sout]scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
