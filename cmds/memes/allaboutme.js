module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(vars.gifFormats.find(f => f === type.ext))) {
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/allaboutme.png -i assets/white.png -filter_complex "[2:v][1:v]scale2ref[w][allabout];[0:v]split=${overlays.length}${osplit.join('')};${oover.join(';')};[whitest][allabout]overlay=x=0:y=0:format=auto[out]" -map "[out]" -vframes 1 -aspect 350:453 -preset ${findpreset(args)} ${filepath}/output.png`)
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/allaboutme.png -i assets/white.png -map 0:a? -filter_complex "[2:v][1:v]scale2ref[w][allabout];[0:v]split=${overlays.length}${osplit.join('')};${oover.join(';')};[whitest][allabout]overlay=x=0:y=0:format=auto,scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -t ${duration} -aspect 350:453  -preset ${findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
