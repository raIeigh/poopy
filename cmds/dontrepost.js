module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/dontrepost.png -filter_complex "[0:v]scale=${squareS.constraint === 'width' || squareS.constraint === 'both' ? 374 : -1}:${squareS.constraint === 'height' || squareS.constraint === 'both' ? 374 : -1}[frame];[1:v][frame]overlay=x=37+(374/2-w/2):y=160+(374/2-h/2):format=auto[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} ${filepath}/output.png`)
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/dontrepost.png -map 0:a? -filter_complex "[0:v]scale=${squareS.constraint === 'width' || squareS.constraint === 'both' ? 374 : -1}:${squareS.constraint === 'height' || squareS.constraint === 'both' ? 374 : -1}[frame];[1:v][frame]overlay=x=37+(374/2-w/2):y=160+(374/2-h/2):format=auto[oout];[oout]scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
