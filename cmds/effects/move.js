module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(vars.gifFormats.find(f => f === type.ext))) {
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/transparent.png -filter_complex "[1:v][0:v]scale2ref[background][input];[background][input]overlay=x=${x}:y=${y}:format=auto[out]" -map "[out]" -preset ${findpreset(args)} ${filepath}/output.png`)
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/transparent.png -map 0:a? -filter_complex "[1:v][0:v]scale2ref[background][input];[background][input]overlay=x=${x}:y=${y}:format=auto,scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
