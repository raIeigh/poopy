module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -filter_complex "[0]rgbashift=rh=${distance}:bv=${distance}:gh=${-distance},format=rgba[in2];[in2][0]scale2ref[in2][in1];[in1][in2]blend=overlay[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} ${filepath}/output.png`)
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -map 0:a? -filter_complex "[0]rgbashift=rh=${distance}:bv=${distance}:gh=${-distance},format=rgba[in2];[in2][0]scale2ref[in2][in1];[in1][in2]blend=overlay[oout];[oout]scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
