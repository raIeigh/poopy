module.exports = {
    name: ['transition'],
    args: [{"name":"transition","required":false,"specifarg":false,"orig":"{transition (default is fade)}"},{"name":"file","required":false,"specifarg":false,"orig":"{file}"},{"name":"file2","required":false,"specifarg":false,"orig":"{file2}"},{"name":"duration","required":false,"specifarg":true,"orig":"[-duration <seconds (max 10)>]"},{"name":"waituntilend","required":false,"specifarg":true,"orig":"[-waituntilend]"}],
    execute: async function (msg, args) {
        ...
        for (var i in targs) {
            ...
            if (unsupported.find(t => t === arg.toLowerCase())) {
                await msg.channel.send('Sorry, but that transition isn\'t supported...').catch(() => { })
                await msg.channel.sendTyping().catch(() => { })
                return
            }
        ...
        if (duration > 0) {
            if ((filetype.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === filetype.ext))) && (filetype2.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === filetype2.ext)))) {
                ...
                await poopy.functions.execPromise(`ffmpeg -stream_loop -1 -t ${iduration2} -r ${fps2.includes('0/0') ? '60' : fps2} -i ${filepath}/${filename} -stream_loop -1 -t ${iduration2 + (args.find(arg => arg === '-waituntilend') ? duration : 0)} -r ${fps2.includes('0/0') ? '60' : fps2} -i ${filepath}/${filename2} -stream_loop -1 -t ${iduration2} -r ${fps2.includes('0/0') ? '60' : fps2} -i assets/transparent.png -filter_complex "[1:v]scale=-1:${height}[vid];[2:v]scale=${width}:${height}[transparent];[transparent][vid]overlay=x=W/2-w/2:y=H/2-h/2:format=auto[transition];[0:v][transition]xfade=transition=${transition}:duration=${duration >= iduration2 && !(filetype2.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === filetype2.ext))) ? iduration2 : duration},scale='min(400,iw)':min'(400,ih)':force_original_aspect_ratio=decrease,split[gnout][gpout];[gpout]palettegen=reserve_transparent=1[palette];[gnout][palette]paletteuse=alpha_threshold=128[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -t ${iduration2} ${filepath}/output.gif`)
                return await poopy.functions.sendFile(msg, filepath, `output.gif`)
            } else if ((filetype.mime.startsWith('image') && poopy.vars.gifFormats.find(f => f === filetype.ext)) && (filetype2.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === filetype2.ext)))) {
                ...
                await poopy.functions.execPromise(`ffmpeg -stream_loop -1 -t ${iduration + (args.find(arg => arg === '-waituntilend') ? duration : 0)} -r ${fps.includes('0/0') ? '50' : fps} -i ${filepath}/${filename} -stream_loop -1 -t ${iduration} -r ${fps.includes('0/0') ? '50' : fps} -i ${filepath}/${filename2} -stream_loop -1 -t ${iduration} -r ${fps.includes('0/0') ? '50' : fps} -i assets/transparent.png -filter_complex "[1:v]scale=-1:${height}[vid];[2:v]scale=${width}:${height}[transparent];[transparent][vid]overlay=x=W/2-w/2:y=H/2-h/2:format=auto[transition];[0:v][transition]xfade=transition=${transition}:duration=${duration}${iduration - duration > duration ? `:offset=${iduration - duration + (args.find(arg => arg === '-waituntilend') ? duration : 0)}` : ''},split[gnout][gpout];[gpout]palettegen=reserve_transparent=1[palette];[gnout][palette]paletteuse=alpha_threshold=128[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -t ${iduration} ${filepath}/output.gif`)
                return await poopy.functions.sendFile(msg, filepath, `output.gif`)
