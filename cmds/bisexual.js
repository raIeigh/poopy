module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(poopy.vars.gifFormats.find(f => f === type.ext))) {
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/bisexual.png -filter_complex "[1:v][0:v]scale2ref[bi][frame];[frame][bi]overlay=x=0:y=0:format=auto[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} ${filepath}/output.png`)
            ...
            await poopy.functions.execPromise(`ffmpeg -i ${filepath}/${filename} -i templates/bisexual.png -map 0:a? -filter_complex "[1:v][0:v]scale2ref[bi][frame];[frame][bi]overlay=x=0:y=0:format=auto[oout];[oout]scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${poopy.functions.findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
